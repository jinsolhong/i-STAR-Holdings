    ...
    try {
      ...
      if (data.success) {
        setInvitee({
          ...invitee,
          rsvp_status: status,
          qr_token_raw: data.qr_token ?? invitee.qr_token_raw,
        });
      } else {
          ...
          {invitee.rsvp_status === 'pending' ? (
            <div className="space-y-3">
              <button
                ...
                className="btn-brand"
              >
                {rsvpLoading ? <LoadingSpinner size="sm" color="white" /> : (
                  <><CheckCircle2 className="w-5 h-5" /> 참석합니다</>
                )}
              </button>
              <button
            ...
            <div className="space-y-4">
              ...
              <div className={`flex items-center justify-center gap-2 p-3 rounded-xl ${
                invitee.rsvp_status === 'attending' ? 'bg-green-50 text-[#006241]' : 'bg-gray-50 text-gray-600'
              }`}>
              ...
              {/* 변경 버튼 */}
              <button
                onClick={() => handleRsvp(invitee.rsvp_status === 'attending' ? 'declined' : 'attending')}
                disabled={rsvpLoading}
                className="btn-outline text-sm"
              >
                {rsvpLoading ? <LoadingSpinner size="sm" color="#006241" /> : (
                  invitee.rsvp_status === 'attending' ? '참석 취소하기' : '참석으로 변경하기'
                )}
              </button>
            </div>
          )}
